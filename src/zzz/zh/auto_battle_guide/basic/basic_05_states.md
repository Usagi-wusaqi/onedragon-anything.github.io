...
状态表达式(states)，是一个字符串，用于判断当前游戏状态是否满足条件。它是处理方法决策的核心。
...
只传一个值 `{3}` 表示精确等于 3（等价于 `{3, 3}`）。

## 4.自定义状态

以 `自定义-` 为前缀的状态是用户自定义的，可以通过 `设置状态` 指令创建，通过 `清除状态` 指令删除。

自定义状态的核心作用是让不同处理方法之间进行"通信"。
...
```yaml
# 步骤 A: 敌人失衡且当前无活动倒计时 → 初始化12秒倒计时
handlers:
  - states: "[敌人-失衡, 0, 1] & ![自定义-失衡计时, -99, 0]"
    operations:
      - op_name: "设置状态"
        state: "自定义-失衡计时"
        seconds: -12
...
- **自定义时间范围**：可以沿用更早识别到的结果。例如 `[按键-连携技-左, 0, 12]` 表示最近 12 秒内连携过，可粗略判断失衡窗口
