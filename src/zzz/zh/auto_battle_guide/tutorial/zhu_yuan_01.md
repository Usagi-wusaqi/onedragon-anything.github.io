...
- [Yaml文件格式](../basic/basic_01_yaml.md) 学习至 `3.2.多层嵌套`
- [自动战斗简介](../basic/basic_00_intro.md) 学习至 `2.配置文件`。
...
`自动战斗配置`中，会包含多个属性，完整属性见 [这里](../basic/basic_00_intro.md) 的 `2.配置文件`，可以只先粗略了解。
...
请戳这里学习 [处理方法](../basic/basic_04_handlers.md) 完成 `简介` 部分学习。

暂时只需了解 `状态表达式(states)` 和 `操作指令(operations)`。

`处理方法` 中，包含多个元素，每个元素是一个 `处理方法` 对象。

上述例子的场景中，主要区分了3种`处理方法`：击破在前场；在后台1；在后台2。

每种`处理方法`下再有对应的`操作指令`。

脚本会按顺序，对每个`处理方法`的`状态表达式`进行判断，匹配到第一个为真的`状态表达式`，然后执行对应的`操作指令`。

#### 3.2.4.状态判断表达式 (states)

请戳这里学习 [状态表达式](../basic/basic_05_states.md) 完成 `基础用法` 部分学习。

#### 3.2.5.操作指令 (operations)

请戳这里学习 [操作指令](../basic/basic_06_operations.md) 完成 `基础用法` 部分学习。
