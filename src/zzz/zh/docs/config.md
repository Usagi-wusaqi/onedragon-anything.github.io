...
1. - [x] `「设置」-「画面」-「帧率」` 建议保持为 `60帧`，画质越高，识别率越高
    - 低帧数低画质无限帧可能会导致识别出错
...
1. - [x] `「设置」-「输入」-「镜头自动跟随转动」`设置为`「关闭」`，功能[拿命验收](./feat_game_assistant.html#拿命验收)模拟按键以此为基础编写
1. - [x] `「设置」-「其他」-「战斗伤害跳字显示」` 设置为 `关闭` 或 `伤害跳字整合` 可以避免部分情况下伤害数字遮挡画面
...
2. - [x] `「设置」-「游戏按键」` 需要根据你游戏中的按键修改设置（默认和游戏默认一致）
...
| [零号空洞](./feat_hollow_zero.md) | 自动完成零号空洞任务（含枯萎之都、迷失之地） |
| [锄大地](./feat_world_patrol.md) | 自动锄大地（打野怪），获得丁尼、代理人经验和音擎经验材料 |
